...
# Global state
# -----------------------------
aircraft = [{"id": "A3", "pos": [300, 150], "controller": 3, "trail": []}]
nodes = {3: {"state": "IDLE"}}
...
snapshot_mgr = SnapshotManager(
    controller_id=3,   # 🔵 changed from 2 → 3
    peers=[1, 2],      # 🔵 include both other nodes
    send_marker=lambda to, sid: multicast.send({"type": "MARKER", "to": to, "sid": sid}),
    emit_snapshot=lambda sid, payload: print(f"[SNAPSHOT] {sid} from controller 3:\n{payload}"),
...
heartbeat = HeartbeatManager(peers=["http://127.0.0.1:5001", "http://127.0.0.1:5002"], timeout=5)
...
# Register self
registry.add_node(3, "http://127.0.0.1:5003")   # 🔵 changed port
...
def simulate():
    while True:
        for a in aircraft:
            a["pos"][0] = (a["pos"][0] + 4) % 600
            a["pos"][1] = (a["pos"][1] + 2) % 400
            a["trail"].append(tuple(a["pos"]))
            if len(a["trail"]) > MAX_TRAIL:
                a["trail"].pop(0)

        if nodes[3]["state"] == "IDLE":
            enqueue(3)
            q = get_queue()
            if q and q[0] == 3:
                set_node_state(3, "CRITICAL_SECTION")
...
if __name__ == "__main__":
    ...
    app.run(port=5003, debug=True)   # 🔵 changed port
