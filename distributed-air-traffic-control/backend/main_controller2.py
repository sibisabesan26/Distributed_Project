...
# Global state
# -----------------------------
aircraft = [{"id": "A2", "pos": [300, 150], "controller": 2, "trail": []}]
nodes = {2: {"state": "IDLE"}}
...
snapshot_mgr = SnapshotManager(
    ...
    emit_snapshot=lambda sid, payload: print(f"[SNAPSHOT] {sid} from controller 2:\n{payload}"),
...
heartbeat = HeartbeatManager(peers=["http://127.0.0.1:5001"], timeout=5)
...
# Register self
registry.add_node(2, "http://127.0.0.1:5002")
...
def simulate():
    while True:
        ...
        if nodes[2]["state"] == "IDLE":
            enqueue(2)
            q = get_queue()
            if q and q[0] == 2:
                set_node_state(2, "CRITICAL_SECTION")
